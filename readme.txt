SQP Fetcher
This script fetches Sponsored Query Performance (SQP) data from Amazon's Brand Analytics API for specified ASINs, ISOs, and date range.
...
Step by step:
Open the following page and login: Seller Central
Open Developer Tools by pressing F12 (or right-click and inspect in Chrome) and go to the Network tab
Reload the page, filter for XHR requests and click on either of the following requests: dashboard, reports
Scroll down to the Request Headers and copy the entire cookie string. Be careful not to copy any other characters before or after the cookie string.
Now that you have obtained the cookie, you can open a command prompt in the downloaded folder and run the command
Usage
...
--cookie-source: Cookie source (optional, choices: 'EU', 'US', 'FE', default: 'EU')
...
Note
Make sure to provide a valid cookie string for the specified cookie source to authenticate the API requests.
